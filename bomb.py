...
def main():
    ...
    if choice == "Home":
        st.subheader("Home")
        image_file = st.file_uploader("Upload Image", type=["jpg", "png", "bmp", "jpeg"])
        if image_file is not None:
            # To See Details
            # st.write(type(image_file))
            # st.write(dir(image_file))
            ####FOR TWO PICTURES PER UPLOAD
            ...
            if docx_file is not None:
                ...
                if docx_file.type == "text/plain":
                    ...
                    raw_text = str(docx_file.read(),
                                   "utf-8")  # works with st.text and st.write,used for further processing
