...
st.set_page_config(page_title="Advance Health Care", page_icon="💊", layout='wide')
...
def main():
	...
	html_temp = """
					        <div style = "background-color:royalblue;padding:5px;border-radius:40px;width :auto;">
					        <h1 style = "color:white;text-align:center;font-size:20px;">Advanced Health Care </h1>
					        </div>
					        """
	components.html(html_temp)

	st.title("Advanced Health Care System")

	menu = ["Home","Login","SignUp"]
	image_lottie_animation = Image.open(r"C:\Users\ADMIN\Pictures\advance1 (1).png")
	st.sidebar.image(image_lottie_animation, output_format="auto", width=100)
	choice = st.sidebar.selectbox("Menu",menu)

	if choice == "Home":
		A, B = st.columns(2)
		with A:
			image_lottie_animation = Image.open(r"C:\Users\ADMIN\Pictures\health3.png")
			st.image(image_lottie_animation, output_format="auto", width=500)
			...
			with B:
				st.write("""
				                                    ...
				                                    **Services🏛️:**
				                                    ...
				                                    - Observe Datasets
				                                    - Self  Testing on Diabetes
				                                    - Self Testing On Heart
				                                    - Self Testing On Parkinson 
				                                    - Education About About Them Three(Diabetes,Heart and Parkinson) 
				                                    - Testing Results 
				                                    - Contact us[Emailing]

				                                    **Meta Charaters:**
				                                    - Leveling and character interaction.

				                                    **Technology🌎:**

				                                     - Artificial Intelligence
				                                     - Machine Learning  models (Logistic Regression,Linear Regression,Decision Tree)
				                                     - Deep Learning models(SVM,LNN,RNN,NLP,ANN,NNI etc)
		...
		institution = st.sidebar.text_input("Institution", placeholder="Enter your school or company")
		address = st.sidebar.text_input("Address", placeholder='Enter address')
		...
		if st.sidebar.checkbox("Login"):
			...
			if result:
				...
				st.success(" Welcome, You are logged in as {}.Please EXPLORE with us!".format(username))
				...
				with A:
					...
					st.markdown('<p class="font">Get Tested with Advance Health Care Team:</p>', unsafe_allow_html=True)
					with B:
						# lottie_coding = load_lottieurl("https://assets7.lottiefiles.com/private_files/lf30_6qyxpnwe.json")
						# st_lottie(lottie_coding, height=100, key="care")
						image_lottie_animation = Image.open(r"C:\Users\ADMIN\Pictures\advance1 (1).png")
						# lottie_coding = load_lottieurl("https://assets7.lottiefiles.com/private_files/lf30_6qyxpnwe.json")
						# st_lottie(lottie_coding, height=100, key="caring")
						st.image(image_lottie_animation, output_format="auto", width=80)

				bio = option_menu(menu_title="检测并聊", options=
					['主页', 'Education|教育', 'Diabetes Prediction|糖尿病检测',
					 'Heart Disease Prediction|心脏病检测',
					 'Parkinsons Prediction|帕金森病检测', '联系我们'],
									  icons=['bank', 'book-half', 'activity', 'heart', 'person-plus-fill',
											 'send-check-fill'],orientation='horizontal')
				...
				if (bio == 'Diabetes Prediction|糖尿病检测'):
					...
					if st.button('Diabetes Test Result:查结果'):
						with st.spinner('please wait,your network seems to good..........!'):
							time.sleep(4)
						# Warnings ignore
						# warnings.filterwarnings(action='ignore')
						st.set_option('deprecation.showfileUploaderEncoding', False)
						...
						if (diab_prediction[0] == 1):
							...
							st.warning(" STATEMENT: It's discovered that according to  the patient's Diabetes Pedigree Function value,having the record of {}% of sugar,the patient's health status is diabetic bound!!!,PLEASE VISIT 🩺(https://vizhub.healthdata.org/gbd-results/) for consultations and treatment from healthcare professionals and suitable medical practitioners.".format(Glucose))
							st.warning(diab_diagnosis)
						else:

							st.success("STATEMENT: It's discovered that according to the patient's number of Pregnancies and blood pressure rate,she has the{}% of sugar which is very favourable at the moment!!!".format(Glucose))
				...
				if (bio == 'Heart Disease Prediction|心脏病检测'):
					...
					if st.button('Heart Disease Test Result'):
						with st.spinner('please wait for the result, your network seems to be weak!...'):
							time.sleep(5)
						# Warnings ignore
						# warnings.filterwarnings(action='ignore')
						st.set_option('deprecation.showfileUploaderEncoding', False)
				...
				if bio == "Education|教育":
					...
					st_lottie(lottie_coding, height=100, key="diabetes")
					st.write("""
					...
					st_lottie(lottie_coding, height=100, key="parkinson")
					st.write("""
					...
					st_lottie(lottie_coding, height=100, key="heart")
					st.write("""
				...
				if bio == "联系我们":
					...
					st.markdown('<p class="font">Contact Us:</p>',
				...
				if bio == "主页":
					...
					st.write("Example")
					st.write(
						"Before jumping to self test page ,first you can study on how to do it using information given below:")
					st.write(
						"检查和理解下面的数据如何测试自己")
		...
		image_lottie_animation = Image.open(r"C:\Users\ADMIN\Pictures\advance1 (1).png")
		st.image(image_lottie_animation, output_format="auto", width=50)
		st.subheader("Create New Account")
		new_user = st.text_input("User Name", placeholder='Enter valid name')
		email = st.text_input('Email', placeholder='enter email')
		institution = st.text_input("Institution", placeholder="Enter your school or company")
		...
		if st.button("Signup"):
			...
			st.write("Already have an account,go to login page!")
...
if __name__ == '__main__':
	...
	#username = st.sidebar.text_input("User Name", placeholder='Enter valid name')
	#email = st.sidebar.text_input('Email', placeholder='enter email')
	#institution = st.sidebar.text_input("Institution", placeholder="Enter your school or company")
	#address = st.sidebar.text_input("Address", placeholder='Enter address')
	#password = st.sidebar.text_input("Password", type='password', placeholder='Enter password')
